...
namespace PosixSocket {
    ...
    std::expected<unique_fd, std::string> ConnectToUnixSocket(const std::filesystem::path &path) {
        if (!exists(path)) {
        ...
        if (pathString.size() >= sizeof(addr.sun_path)) {
            return std::unexpected{
                std::format("Socket path too long ({} >= {})", pathString.size(), sizeof(addr.sun_path))
            };
        }
